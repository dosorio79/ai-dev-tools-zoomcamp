import type { ExecutionResult, Language } from "@/api";
...
const injectPyodideScript = (base: string) =>
  new Promise<void>((resolve, reject) => {
    ...
    script.onerror = () =>
      reject(new Error(`Pyodide script failed to load from ${script.src}. Set VITE_PYODIDE_BASE if hosting locally.`));
...
export const preloadPyodide = async () => loadPyodideInstance().catch((err) => {
  pyodideLoading = null;
  throw err;
});
...
import sys, io, traceback, textwrap, json
_buffer = io.StringIO()
...
export const executeLocally = async (language: Language, code: string): Promise<ExecutionResult> => {
  if (language === "python") {
